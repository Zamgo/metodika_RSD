---
title: Fakturace
typ: workflow
faze: []
role: [technicky dozor, zhotovitel]
cinnosti: [schvalovani]
workflow: [fakturace]
temata: [workflow, cde]
stav: draft
vlastnik: rsd
souvisi: []
priloha: []
permalink: /workflow/fakturace
aliases: []
tags: [workflow]
---

## Účel workflow

Schválení Fakturace

## Kdy se spouští

**Editorske doplneni:** V Excel šabloně není explicitně uvedeno; doplnit dle metodiky projektu.

## Vstupy

**Editorske doplneni:** Dle šablony nejsou explicitně uvedeny; doplnit dle metodiky projektu.

## Role

- [[Zhotovitel]]
- [[Technický dozor]]

## Hlavní kroky

| Krok | Název kroku | Odpovědná strana | Deadline | Popis kroku |
|---:|---|---|---|---|
| 1 | Předložení Záznamu o měření položek | Zhotovitel | Do 25. dne v měsíci | Zhotovitel nahraje Záznam o měření položek dle Metodiky měření. |
| 2 | Schválení TDS | SO XXX | 3 dny | TDS zkontroluje ZMP a opatří elektronickým podpisem. |
| 3 | Předložení Zjišťovacího protokolu | Zhotovitel | 2 dny | Zhotovitel nahraje Zjišťovací protokol dle Metodiky měření. |
| 4 | Schválení TDS | SO XXX | 2 dny | TDS zkontroluje ZP a opatří elektronickým podpisem. |
| 5 | Nahrání do DSS AspeHub | Proconom | 3 dny | Pracovník společnosti Proconom nahraje podepsané dokumenty do AspeHub a předá info o nahrání. |

## Výstupy

**Editorske doplneni:** Odvodit z posledního kroku workflow a vazby na CDE (např. schválený dokument ve stavu Schváleno).

## Vazba na CDE

Workflow je realizováno prostřednictvím CDE (schvalování, připomínkování, auditní stopa).

## Vazba na příslušnou přílohu nebo šablonu

- [[Šablony workflow (P1b)]]

## Související role

- [[Zhotovitel]]
- [[Technický dozor]]

## Související fáze

- [[Příprava]]
- [[Realizace]]
- [[Provoz a údržba]]

## Související pojmy

- [[Společné datové prostředí (CDE)]]
- [[Auditní stopa]]
- [[Informační kontejner]]
